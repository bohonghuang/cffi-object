...
(defmacro define-prototype-cobject ((name type) &aux (*package* (symbol-package name)))
...
(defmacro define-struct-cobject ((name &optional ctype) &aux (*package* (symbol-package name)))
...
(defmacro define-type-cobject ((name type) &aux (*package* (symbol-package name)))
  (let* ((type (typecase type (cffi::foreign-type type) (t (cffi::parse-type type))))
         (base-type (cffi::ensure-parsed-base-type type)))
    (with-new-cobject-class-definition (name type)
...
(defmacro define-package-cobject ((target &optional source))
  (unless source
  ...
  (loop :with source-package := (find-package source) :and target-package := (find-package target)
        ...
        :for source-name :being :each hash-key :of cffi::*default-type-parsers* :using (hash-value type-getter)
        :when (eql (symbol-package source-name) source-package)
          :do (symbol-macrolet ((name (intern (symbol-name (cffi::name type)) target-package)))
                (labels ((push-definition (type)
                  ...
                  (push-definition (ignore-some-conditions (warning) (funcall type-getter)))))
